package com.gmail.Haggard_nl.ChestProtect.Commands;
...
public class ChestLockCommand implements CommandExecutor {
    ...
    public static void list(Player player, ListType type) {
        ...
        if (!ChestProtect.hasPermission(player, "list."+type)) {
        ...
        switch (type) {
            ...
            case CHESTS: //List Chests that the Player Owns
                LinkedList<Safe> ownedChests = ChestProtect.getOwnedChests(player.getName());
                
                //Display amount of Chests owned
                String chestMsg = "You own "+ownedChests.size()+" Chests";
                
                //Display the limit if there is one
                int chestLimit = ChestProtect.getOwnLimit(player, "chest");
                if (chestLimit > -1)
            ...
            case FURNACES: //List Furnaces that the Player Owns
                LinkedList<Safe> ownedFurnaces = ChestProtect.getOwnedFurnaces(player.getName());
                
                //Display amount of Furnaces owned
                String furnaceMsg = "You own "+ownedFurnaces.size()+" Furnaces";
                
                //Display the limit if there is one
                int furnaceLimit = ChestProtect.getOwnLimit(player, "furnace");
                if (furnaceLimit > -1)
            ...
            case DISPENSERS: //List Dispensers that the Player Owns
                LinkedList<Safe> ownedDispensers = ChestProtect.getOwnedDispensers(player.getName());
                
                //Display amount of Dispensers owned
                String dispenerMsg = "You own "+ownedDispensers.size()+" Dispensers";
                
                //Display the limit if there is one
                int dispenerLimit = ChestProtect.getOwnLimit(player, "dispener");
                if (dispenerLimit > -1)
            ...
            case DOORS: //List Doors That the Player owns and what their key is
                LinkedList<LockedDoor> ownedDoors = ChestProtect.getOwnedDoors(player.getName());
                
                //Display amount of Dispensers owned
                String doorMsg = "You own "+ownedDoors.size()+" Doors";
                
                //Display the limit if there is one
                int doorLimit = ChestProtect.getOwnLimit(player, "door");
                if (doorLimit > -1)
            ...
            case CLEAR: ChestProtect.clear(player.getName()); player.sendMessage(ChestLockMessages.getClearMsg()); break;
    ...
    public static void coowner(Player player, CoOwnerType type, Add_Remove action, String coOwner) {
        //Cancel if the Player does not have permission to use the command
        if (!ChestProtect.hasPermission(player, "coowner")) {
            player.sendMessage(ChestLockMessages.getPermissionMsg());
            return;
        }
    ...
    public static void lock(Player player) {
        ...
        if (door == null) {
            ...
            int limit = ChestProtect.getOwnLimit(player, "door");
            if (limit > -1 && ChestProtect.getOwnedDoors(player.getName()).size() >= limit) {
                player.sendMessage(ChestLockMessages.getLimitMsg(block.getType()));
                return;
            }  
            
            ChestProtect.doors.add(new LockedDoor(player.getName(), block, id));
        }
        else if (!door.owner.equals(player.getName()) && !ChestProtect.hasPermission(player, "admin")) {
            player.sendMessage(ChestLockMessages.getDoNotOwnMsg(block.getType()));
            return;
        }
        else
